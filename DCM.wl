...
maxStationary::usage = "maxStationary[g] returns {pimax,piDelta,attractive} 
where 
...
inWeight::usage = "inWeight[g,pi,vs] computes the weight of the in-neighbourhood \
of vs"


simDCM::usage = "simDCM[pdf,r,n,p,nsample] simulate DCM with in-degree distribution \
pdf, out-degree r, size n, with max degree cutoff probability p"
...
findMaxDegree[pdf_,n_/;IntegerQ[n]&&n>=1,p_/;1>=p>0]:=Module[{deg=1,p0=1},
While[n*p0>=p,
deg=deg+1;
p0=pdf[deg];
];
...
isAttractive[g_,v_]:=Module[{n},
n=VertexCount[g];
Length[VertexInComponent[g,v]]==n
]


deg2Stub[degs_/;ArrayQ[degs,1,IntegerQ]]:=Module[{nedges,stubs,m=0},
nedges=Total[degs];
...
heavyTailDist[a_/;a>1,r_/;r>0]:=Module[{prob1,pdf1,mean1,p,p1,prob2,pdf2},
...
pdf2Deg[pdf_,n_/;IntegerQ[n]&&n>=1]:=Module[{n1,deglist,t,i,d,nhead,ntail,diff,heads,tails},
n1=n;
deglist={};
For[t=0,True,t++,
For[i=0,i<=t,i++,
(* the number of nodes with degree {i,t-i} *)
...
pdf2Deg[pdf_,r__/;IntegerQ[r]&&r>=1,n_/;IntegerQ[n]&&n>=1,p_/;0<p<=1]:=Module[
{maxdeg,nedge,d,dn,deglist},
...
If[nedge>0,deglist[[;;nedge]]+=1];
{deglist,ConstantArray[r,n]}
]


makeDCM[deglist_]:=Module[{heads,tails},
    {heads,tails}=deg2Stub /@ deglist;
    makeDCM[Length[deglist[[1]]],heads,tails]
]

makeDCM[n_Integer,heads_List,tails_List]:=Module[{},
Graph[Range[n],Thread[tails\[DirectedEdge]RandomSample[heads]]]]

transitProbM[g_]:=Module[{adj,outdeg,transit},
...
stationaryProbEigen[g_]:=Module[{transit,pi,n,ev},
n=VertexCount[g];
...
stationaryProbNull[g_]:=Module[{transit,pi,n,ev},
n=VertexCount[g];
transit=Transpose[transitProbM[g]];
...
stationaryProb[g_]:=Module[{n,mp,stationary,pi},
n=VertexCount[g];
...
stationaryProbSim[g_]:=Module[{n,steps},
n=VertexCount[g];
...
{n,pi,indeglist,vDelta,piDelta,vpimax,pimax,attractive},
n=VertexCount[g];
...
pimax=pi[[vpimax]];
attractive=isAttractive[g,vpimax];
{pimax,piDelta,attractive}]

simDCM[pdf_,r_,n_,p_,nsample_] := Module[
{degseq, heads, tails, data, maxdeg, col,m},
m=n*r;
degseq = pdf2Deg[pdf, r, n, p];
maxdeg = Max[degseq[[1]]];
{heads, tails} = deg2Stub /@ degseq;
data = ParallelTable[ 
    maxStationary[makeDCM[n, heads, tails]],
    {i, 1, nsample}];
...
simAnalyze[dt_] := Module[{dt1, dt2, dt3},
dt1 = dt["data"];
dt2 = dt1[Select[#attractive == True &]][
All, {"pimax", "piDelta"}];
dt3 = dt[{"n", "m", "maxdeg"}]~Join~Mean[dt2];
dt3["\[CapitalDelta]/m"] = (#["maxdeg"]/#["m"]) &@dt // Normal // 
N;
dt3["piDelta/(\[CapitalDelta]/m)"] = (#["piDelta"]/#[
        "\[CapitalDelta]/m"]) &@dt3 // Normal // N;
dt3["pimax/(\[CapitalDelta]/m)"] = (#["pimax"]/#[
        "\[CapitalDelta]/m"]) &@dt3 // Normal // N;
dt3
]
